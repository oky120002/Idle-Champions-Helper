...
- normalize/build 数据管线增量跳过：`normalizeDefinitionsSnapshot` 与 `buildModels` 对比 raw `checksum`（稳定数据指纹，优先；游戏数据没变时不变）或 `current_time`（fallback）与产物记录、以及数据管线源码指纹 `pipelineHash`（`scripts/data` + `src/domain/abilities` + `src/domain/effects` 下非 test 的 .ts + normalize/fetch/build 三入口的 sha256）——raw 数据未变（checksum 同）且逻辑未变则跳过重生成。`checksum` 优先于 `current_time`：后者每次 fetch 单调递增（即使游戏数据没变），曾导致空重写（~191 产物纯时间戳噪音）。`pipelineHash` 自动检测 `scripts/data` + `src/domain/abilities` + `src/domain/effects` + 三入口的源码改动（无需手动 force）——这两个 domain 目录的全部源文件都是数据管线 build 依赖（被 effect-helpers / effect-resolvers / feat-catalog / specialization-catalog 导入），改归一化语义自动触发重建。`FORCE_DATA_REBUILD=1` 作通用逃生口（如怀疑指纹漏检或产物异常）。fetch 受限于「必须下载才知 current_time」，无法下载前跳过。

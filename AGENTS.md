...
- 数据源格式特性优先在归一化层（`normalize-idle-champions-definitions.mjs`）适配，让消费层拿干净数据；无法在归一化层处理的才退到消费层防御。已确认的数据格式特性清单见 `docs/research/data/game-data-source/format-quirks.md`。

...
- 大型任务（覆盖文档、测试、开发等各类工作）开工前必须先按「文档落库」要求在 `docs/plans/` 落库执行计划，随后按计划推进；计划与实际偏离时同步更新计划文档，保持其始终是当前执行依据。
- 大型任务完成后必须执行**文档归档收尾**（非可选清理）：计划所有 checklist 完成并验证通过后，同步更新 `specs/` 描述新现状，计划标 `Landed` 移入 `archives/plans/`，关联的需求加终态标记移入 `archives/requirements/`（细节见 `governance.md` §8「怎么更新」）。
- 小型任务可直接开发，无需计划落库。
- 大型/小型由执行智能体自行判断；参考信号：跨多模块、多步骤、影响面广或需要中途审计追溯的倾向大型。拿不准时按大型处理。

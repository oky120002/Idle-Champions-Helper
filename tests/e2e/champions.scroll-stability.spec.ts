...
async function clickVisibleResultLink(page: Page): Promise<void> {
  ...
  // Playwright's auto-scroll can perturb the pane position before the app saves it.
  await page.locator('.results-grid .result-card--link').nth(linkIndex).evaluate((element) => {
    if (!(element instanceof HTMLElement)) {
      throw new Error('结果卡链接不存在。')
    }

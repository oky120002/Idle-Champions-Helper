...
test('变体限制页首次显示当前筛选摘要时不应推挤结果说明位置', async ({ page }) => {
  ...
  const content = page.locator('.surface-card__body')
  const baselineTop = await getRelativeViewportTop(summary, content)

  await page.getByRole('combobox').selectOption({ index: 1 })
  await page.waitForTimeout(100)

  expect(Math.abs((await getScrollY(page)) - baselineScrollY)).toBeLessThanOrEqual(2)
  expect(Math.abs((await getRelativeViewportTop(summary, content)) - baselineTop)).toBeLessThanOrEqual(2)
})

...
test('GitHub Pages 基线路径下可以完成主页与个人数据 smoke 回归', async ({ page }) => {
  ...
  await expect(page.locator('.champions-workbench__toolbar-title')).toHaveText('英雄筛选')
  await expect(page.getByRole('button', { name: '显示全部' })).toBeVisible()

  await page.getByRole('link', { name: '个人数据' }).click()
  await expect(page.locator('.user-data-workbench__toolbar-title')).toHaveText('个人数据')

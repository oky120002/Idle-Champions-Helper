...
- `shared/` 只放跨页面稳定契约；`surfaces/`、`filters/`、`workbench/`、`results/`、`formation/` 都按“基础块 / 场景块 / 响应式”拆分。
...
- 通用表面：`src/styles/shared/surfaces/panels.css` -> `page-header-metrics.css` -> `section-primitives.css` -> `responsive.css`
- 筛选：`src/styles/shared/filters/base.css` -> `active-filter-bar.css` -> `sidebar.css` -> `disclosure.css`
- 工作台壳层：`src/styles/shared/workbench/shell.css`

...
export function AnimationAuditPage() {
  ...
  const toolbarItems: WorkbenchToolbarItemConfig[] = [
    createWorkbenchBadgeItem({ id: 'flagged', label: t("待复核 {p0}", { p0: String(model.summary.flagged) }) }),
    createWorkbenchBadgeItem({ id: 'high', tone: 'muted', label: t("高疑似 {p0}", { p0: String(model.summary.high) }) }),
    createWorkbenchBadgeItem({ id: 'medium', tone: 'muted', label: t("中疑似 {p0}", { p0: String(model.summary.medium) }) }),
  ...
  const showAllLabel = model.showAll ? t("收回短名单") : t("展开全部结果")
  ...
  return (
    <ConfiguredWorkbenchPage
      ...
      floatingTopButton={showScrollTop ? { onClick: scrollToTop, detailLabel: t("动图审片内容") } : undefined}
      toolbar={{
        sections: [
          {
            region: 'lead',
          ...
          },
          {
            ...
            section: {
              ...
              kicker: t("人工审片"),
              title: t("英雄动图审片台"),
              detail: t("把当前默认 sequence 与更像 idle 的候选并排放在一起，只看短名单。"),
            },
          },
          {
            region: 'actions',
      ...
      <WorkbenchContentStack>
        <section className="surface-card animation-audit-controls">
          <div className="surface-card__header">
            <div className="surface-card__header-copy">
              <p className="surface-card__eyebrow">{t("审片控制")}</p>
              <h2 className="surface-card__title">{t("先看高疑似，再决定要不要扩表")}</h2>
              <p className="surface-card__description">
                {t("每行只有当前默认、推荐候选和一个备选。点“播放这一行”后，就能并排看哪一个更接近游戏里的待机。")}
              </p>
            </div>
          </div>
          <div className="surface-card__body animation-audit-controls__body">
            ...
            <label className="animation-audit-search">
              ...
              <input
                ...
                placeholder={t("例如：Strix、斯崔克丝、hero:38")}
              />
            </label>
        ...
        {model.state.status === 'loading' ? (
          <section className="surface-card animation-audit-state-card">
            <div className="surface-card__body">{t("正在加载动图审计清单…")}</div>
          </section>
        ) : null}
        ...
        {model.state.status === 'error' ? (
          <section className="surface-card animation-audit-state-card">
            <div className="surface-card__body">
              {t("无法读取动图审计清单：{p0}", { p0: model.state.message })}
            </div>
          </section>
        ) : null}
        ...
        {model.state.status === 'ready' ? (
          <section className="animation-audit-results">
            <div className="animation-audit-results__header">
              <div>
                <p className="animation-audit-results__eyebrow">{t("短名单")}</p>
                <h2 className="animation-audit-results__title">
                  {t("当前显示 {p0} 行", { p0: String(visibleRows.length) })}
                </h2>
                <p className="animation-audit-results__description">
                  {t("默认先只给你短名单；如果这一轮不够，再点展开。")}
                </p>
              </div>
              {model.canShowMore ? (
                <button
            ...
            {missingAnimationCount > 0 ? (
              <p className="animation-audit-results__notice">
                {t("{p0} 行缺少基础动画清单，已跳过展示。", { p0: String(missingAnimationCount) })}
              </p>
            ) : null}

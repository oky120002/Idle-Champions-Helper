...
1. `src/pages/PresetsPage.tsx`
   - 先看页面如何接入全站工作台壳层、工具条和右侧内容区；“当前范围 / 当前边界”说明块与列表空态也已经直接并回这里。
2. `src/pages/presets/usePresetsPageModel.ts`
   - 看读取、刷新、编辑、删除、恢复的状态与动作装配。
3. `src/pages/presets/PresetCard.tsx`
   - 看单条方案卡片、兼容恢复提示、删除确认与编辑入口。
4. `src/pages/presets/PresetEditorForm.tsx`
   - 看名称、备注、标签、优先级编辑表单。
5. `src/pages/presets/preset-model.ts`
...
- `PresetsPage.tsx`: 同时承接页面级说明块、列表空态和列表容器，避免再拆只做转发的超薄 wrapper。

...
describe('HeroPicker', () => {
  ...
  it('picker 模式英雄卡为 button（可点击选择），不可拖', async () => {
    ...
    await user.click(screen.getByTestId('hero-picker-trigger'))
    const jimCard = unwrap(screen.getByText('吉姆').closest('button'), 'expected jim card button')
  ...
  it('拖拽源模式（不传 onChange）：trigger 显示拖拽提示、不渲染未放置、英雄卡为 div 且 draggable', async () => {
    ...
    const jimCard = unwrap(screen.getByText('吉姆').closest('div'), 'expected jim card div')

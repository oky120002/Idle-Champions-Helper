 ...
 * 产 equipmentMult（真实）与 theoreticalLootMult（理论上界），供调整比：
...
/**
 * 理论 loot multiplier = `1 + Σ(全 slot 全 rarity DPS effect)/100`。
 * 即当前 hero-abilities.json 基线（collectRawEffectEntries 全量 loot 累加）的等价量。
 * 供计算调整比 equipmentMult / theoreticalLootMult。
 */
...
/**
 * 装备调整比 = ownedEquipMult / theoreticalLootMult。
 * 用于把理论基线 carryDps 缩放到玩家实际装备：

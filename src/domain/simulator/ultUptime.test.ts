...
describe('computeUltUptime', () => {
  ...
  it('modron 未激活 → uptime = 0（保守不计 ult buff）', () => {
    expect(computeUltUptime(30, 900, false)).toBe(0)
  })

  it('duration = 0（瞬时 ult）→ uptime = 0', () => {
    expect(computeUltUptime(0, 3600, true)).toBe(0)
  })

  it('uptime 上限 1（duration ≥ cooldown 时满覆盖）', () => {
    expect(computeUltUptime(100, 50, true)).toBe(1)
  })
...
describe('foldUltBuffValue', () => {
  ...
  it('uptime = 0 → 有效值 0（不计入）', () => {
    expect(foldUltBuffValue(100, 0)).toBe(0)
  })
})

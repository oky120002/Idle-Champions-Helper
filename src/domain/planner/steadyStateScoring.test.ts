...
describe('steady state scoring', () => {
  ...
  it('team-speed 模式动态速度英雄使用默认值参与计算', () => {
    ...
    expect(result.warnings[0]).toHaveProperty('literal', expect.stringContaining('布里夫'))
    expect(result.warnings[0]).toHaveProperty('literal', expect.stringContaining('25%'))
  })
  ...
  it('team-speed 模式动态速度英雄支持入参覆盖', () => {
    ...
    expect(result.warnings[0]).toHaveProperty('literal', expect.stringContaining('50%'))
  })

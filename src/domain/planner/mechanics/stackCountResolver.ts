...
export const STACK_COUNT_RESOLVERS: Record<string, {
  ...
  contextLabel: MessageRef
}> = {
  // 机制: formation-count-mult-stack / formation-count-add-stack（per_hero 是 per_crusader 同义词）
  per_crusader: { count: (input, signal) => countQualifiedHeroes(input, signal), contextLabel: { key: '整队计数' } },
  per_hero: { count: (input, signal) => countQualifiedHeroes(input, signal), contextLabel: { key: '整队计数' } },
  per_tagged_crusader_mult: { count: (input, signal) => countQualifiedHeroes(input, signal), contextLabel: { key: '整队计数' } },
  per_target_crusader: { count: (input, signal) => countQualifiedHeroes(input, signal), contextLabel: { key: '整队目标计数' } },
  per_hero_attribute: { count: (input, signal) => countQualifiedHeroes(input, signal), contextLabel: { key: '整队属性计数' } },
  per_upgrade_targets: { count: (input, signal) => countUpgradeTargets(input, signal), contextLabel: { key: '整队目标' } },
  // 机制: topology-count-stack（列/槽位距离拓扑计数）
  per_col_behind: { count: (input) => countColumnsCarryBehindSupport(input), contextLabel: { key: '阵型列拓扑' } },
  per_slot_distance_from_source: { count: (input) => countSlotDistanceFromSource(input), contextLabel: { key: '阵型槽位距离' } },
}

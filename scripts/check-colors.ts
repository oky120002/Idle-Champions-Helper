...
// ponytail: 拆两条 regex 规避 sonarjs regex-complexity（合并版 21 超阈值 20）；语义等价于原合并 RE。
const COLOR_FUNC_RE = /(?:rgba?|hsla?|oklch|oklab|lab|lch)\(\s*\d/
  ...
  function scan(dir: string) {
    for (const entry of readdirSync(dir)) {
      ...
      lines.forEach((line, index) => {
        ...
        if (COLOR_FUNC_RE.test(line) || HEX_COLOR_RE.test(line)) {
...
function main() {
  ...
  if (violations.length > 0) {
    console.error(
      `✗ 发现 ${String(violations.length)} 处硬编码颜色。颜色必须登记为 token（src/styles/foundations/tokens.css），禁止在业务 CSS 写 rgb()/rgba()/#hex：`,
    )
...
if (process.env.VITEST == null || process.env.VITEST === '') {
  main()
}

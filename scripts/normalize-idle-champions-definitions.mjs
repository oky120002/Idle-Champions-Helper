...
可选参数：
  ...
  --output-dir <dir>         归一化集合输出目录，默认 ${DEFAULT_OUTPUT_DIR}
  --version-file <file>      version.json 输出位置，默认 ${DEFAULT_VERSION_FILE}
  --current-version <name>   version.json 中的 current 字段，默认 ${DEFAULT_CURRENT_VERSION}
  --manual-overrides <file>  手工补充层 JSON，默认 ${DEFAULT_MANUAL_OVERRIDES}
  --help                     显示帮助
`)
}

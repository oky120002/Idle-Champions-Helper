...
function printUsage() {
  console.log(`用法：
  node scripts/fetch-idle-champions-definitions.mjs [--out-dir <dir>] [--master-api-url <url>]

可选参数：
  --out-dir <dir>                    原始 definitions 快照输出目录，默认 ${DEFAULT_OUT_DIR}
  --master-api-url <url>             Play server 发现接口根地址
  --playserver-client-version <n>    getPlayServerForDefinitions 使用的客户端版本，默认 ${DEFAULT_PLAYSERVER_CLIENT_VERSION}
  --definitions-client-version <n>   getDefinitions 使用的客户端版本，默认 ${DEFAULT_DEFINITIONS_CLIENT_VERSION}
  --help                             显示帮助
`)
}

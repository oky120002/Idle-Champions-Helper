...
**出战栏位**：
英雄所属的游戏栏位编号（bench seat），是英雄的固有属性；同栏位英雄不能同时上场（互斥约束），不是阵型位。
代码标识符：`seat`（英雄数据字段）
别名：座位、位置、slot（社区习惯词；与阵型位是不同概念）
...
**勇士**：游戏机制文本中对 Champion 的称呼，指可加入队伍并放入阵型的角色；“英雄”是官方中文文本和社区中的常用称呼。
代码标识符：`hero`、`heroes`、`Hero`、`hero_id`
别名：champion、英雄

**位置阵型技能**：英雄对阵型中其他位置或整队产生的技能效果，官方文本对应 formation ability；与普通自身技能效果区分。
代码标识符：`formation_ability`、`buff_*_formation_abilities`
别名：阵型技能、阵型效果、formation ability

**金币寻获量**：官方游戏文本中的 gold find，表示队伍获得金币的加成；“金币发现”是社区和项目中的习惯词。
代码标识符：`gold_multiplier_mult`、`computeTeamGoldFind`
别名：金币发现、gold find
...
**神恩**：
战役完成后积累的永久货币，每点未花费的神恩提供 +1% 全队金币寻获量。可在祝福上花费（花掉后不再计金币加成）。不同战役有各自的神恩，互不通用。
代码标识符：`favor`（`CampaignFavorBlessings.favor` 存储字段）
别名：恩赐、divine favor、favor

...
| 全部 Vitest | `npm run test:run` |
| 数据与 schema | `npm run test:data` |
| 浏览器流程 | `npm run test:e2e` |
| 发布前完整回归 | `npm run test:regression` |

所有测试脚本都先执行类型检查。定位单个 Vitest 文件时使用现有脚本传参：`npm run test:unit -- scripts/docs-governance.test.ts`。

## 浏览器验收前

1. 运行 `npm run build` 生成当前产物。

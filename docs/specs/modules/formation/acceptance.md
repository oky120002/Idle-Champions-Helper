...
- 已实现：初始化读取最近草稿；三种恢复处理入口；`layoutId / placements / scenarioRef` 防抖写入 `IndexedDB`；优先按保存版本恢复；兼容恢复显式提示；清空空阵时自动清理最近草稿；从阵型页「保存为方案」。
...
- 验证：`npm run build` + `lint` 通过；浏览器自动化覆盖「放英雄 → 自动保存 → 刷新恢复 → 保存为方案 → 方案恢复 → 删除方案」主链路；受限会话 Playwright 边界见 `docs/archive/investigations/runtime/playwright-browser-launch/README.md`

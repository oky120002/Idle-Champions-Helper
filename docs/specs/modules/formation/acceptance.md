...
- 验证：`pnpm run build` + `lint` 通过；浏览器自动化覆盖「放英雄 → 自动保存 → 刷新恢复 → 保存为方案 → 方案恢复 → 删除方案」主链路；受限会话 Playwright 边界见 `docs/archives/investigations/runtime/playwright-browser-launch/README.md`

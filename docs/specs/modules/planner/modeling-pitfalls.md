...
- **runtime 注入修改预计算输入后必须重算预计算值**：`speedProfile.speedGain` 被 `computationMode` 的候选裁剪使用，任何 runtime 注入（专精 `applySpecializationsToProfile` / 装备 buff `applyEquipmentBuffs`）修改了 `speedProfile.effects` 后，必须调用 `computeHeroSpeedGain` 重算 `speedGain`。该不变量适用于所有「build 期预算 → runtime 消费」字段。

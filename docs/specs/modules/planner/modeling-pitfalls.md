...
- **runtime 注入修改预计算输入后必须重算预计算值**：`speedProfile.speedGain` 被 `computationMode` 的候选裁剪使用，任何 runtime 注入（专精 `applySpecializationsToProfile` / 装备 buff `applyEquipmentBuffs`）修改了 `speedProfile.effects` 后，必须调 `computeHeroSpeedGain` 重算 `speedGain`。曾因 spec 注入后硬编码 `speedGain: 1`（注释误称「不影响裁剪」），导致 team-speed 模式下速度英雄在 p50 裁剪中被误删。同一不变量适用于所有「build 期预算 → runtime 消费」字段。

...
## 评分规则
- 主评分只看 `carryDps`（carry-dps 模式）或 `teamGoldFind`（team-gold 模式）：C 位 `baseDamage × levelCurve × 加成聚合`，涵盖自增伤、对 C 位生效的全队增伤、位置增伤、标签 / 阵营 / 职业 / 性别条件增伤，以及会反映到 C 位有效输出的敌方承伤 / 易伤类效果。
...
2. 场景 / 布局硬约束
3. 槽位拓扑事实
4. unsupported / unknown（进 warning，不计分）

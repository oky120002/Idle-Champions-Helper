...
| `buff-upgrade-modifier` | 装备/专长/feat 修饰 | effect kind `buff_upgrade`/`buff_upgrades` | `applySignalPercent` 按基础 `value`（per-stack 百分比）折算，非聚合倍率；依赖基础 multiplier>1（叠层 0 层不生效）（`// 机制: buff-upgrade-modifier`） | 蔚「时髦披肩」「道德规范」 |

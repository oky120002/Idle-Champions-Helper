...
传奇效果评分支持存档驱动和无存档假设两种输入：已锻造槽位或假设目录条目按等级线性缩放，普通全队加成、按英雄条件加成和按阵型人数叠加的效果分别进入统一伤害池；缺少英雄归属、目录或效果无法解析时保守跳过。planner 在当前推荐阵型下按同一目录和阵型标签输出稳定的锻造优先级 TOP 5，不处理重铸概率、神眷管理或传奇专长。

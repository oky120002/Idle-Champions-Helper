...
共享组件模块只承接“跨页面重复、语义稳定、仍会持续复用”的实现，不负责：
...
- 组件目录和文档索引必须同步更新，避免智能体在页面里重复造轮子
- 当四个以上页面已经出现同一类工作台 chrome 时，优先抽“展示骨架”而不是抽页面业务数据；业务 copy 和指标来源继续留在页面 feature 内。
- 共享层稳定后，要反过来清理页面内只做一次转发的超薄 wrapper；10-30 行、且只负责拼两个子块的叶子文件，优先并回邻近入口，避免为了命名再多开一层文件。

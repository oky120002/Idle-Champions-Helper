# 全站页面工作台设计规范

- 作用：固定 `PageWorkbenchShell` 视觉签名、结构语义、滚动模型、关键动效与页面归位映射，作为细节迭代与跨页复核的设计基线。

## 结构签名

- 桌面端统一使用“双层页面”：
  - 上层是默认紧凑态的全站小导航，像悬浮指挥条，而不是大页头。
  - 下层是单一外层工作台壳体，保留站点背景与光斑，主页面不拆成彼此独立的大卡。
- 工作台内部统一保留“左语义 / 右语义”：
  - 有左栏页面：左侧是抽屉式 pane，右侧是主内容画布。
  - 无左栏页面：只隐藏左 pane，本体仍走同一套右区结构，统一使用同一页面体系。
- 移动端继续退化为普通单列网页滚动，不强行维持桌面高度锁定和双栏内滚。

## 顶部小导航

  - 命中工作台路由时，桌面端头部默认直接进入 condensed 态。
  - 小导航职责限于全站切页、当前工作区识别和低频控制；页面内返回入口、筛选动作和详情页局部操作归属页面工具栏。
- 视觉上保持“重结构、轻图标”：
  - 外层是悬浮战术壳；
  - 导航项是横向标签式切页；
  - 装饰来自切角、边缘光、密度变化，不用渐变文字、彩色侧条和泛滥玻璃化。

## 工作台壳层

- 外层工作台必须是一整块连续材质：圆角比站点头部更小、更克制，边缘更像战术台舱门，不像独立弹窗。
- 左右 pane 都挂在同一个外壳里，不允许再次拆成两个互不相干的大卡。
- 左 pane 材质更轻、更透，像前景抽屉；右 pane 更深、更实，像主内容画布。
- 左右顶部 chrome 高度必须一致，并作为同一条工具带来阅读，不能出现“左边一条、右边再一条”的断裂感。

## 工具条语义

- 工作台工具条统一拆成三段：
  - `toolbarLead`：页面标记、返回入口或左侧识别信息；筛选页默认把“条件待命 / N 项条件”也收进这里；
  - `toolbarPrimary`：页面主标题、kicker、上下文说明；
  - `toolbarActions`：复制当前链接、显示全部 / 收起、随机排序等稳定动作；筛选命中 badge 不放入工具栏，多个动作默认收进一组紧凑的 action cluster。
- 结果区内部优先只保留 compact metrics、筛选摘要和必要状态；高频结果动作（如随机排序、显示全部 / 收起）默认上提到 `toolbarActions`，不要留在结果头内部。
- 这三段的稳定展示骨架统一由 `src/components/workbench/WorkbenchScaffold.tsx` 承接；页面只负责传业务文案、统计值和动作回调。
- 右侧 `toolbarActions` 里的 badge / button / share 默认优先走配置驱动的 `src/components/workbench/WorkbenchToolbarItems.tsx`，避免页面重复手写结构。
- 有左栏时：
  - `toolbarLead` 留在左 pane 顶部；
  - `toolbarPrimary + toolbarActions` 留在右 pane 顶部；
  - 两段需要保持同高、同底色体系和连续边界，像一条被中缝切开的工作带。
- 无左栏时：
  - `toolbarLead` 内联并入右侧工具条左端；
  - 右端动作仍固定靠右；
  - 不渲染空的左轨、占位 gutter 或残余边框。

## 左侧状态

- `展开态`
  - 左工具条和左主体必须是一体化抽屉，不是上面单独一条、下面再接筛选区。
  - 中间分隔只是一条细边界，强调左右职责，不制造裂缝感。
- `收起态`
  - 左抽屉主体、边框、阴影和 gutter 全部消失；
  - 只保留一个贴在左上角的紧凑锚点按钮；
  - 收起后右区立即补满，不留残余窄轨。
- `无左栏态`
  - 不渲染 toggle；
  - 不渲染左 pane；
  - 仍保留同一套右区 chrome、内容头和主滚动模型。

## 动效与过渡

- 抽屉动效目标是“侧层平移”而不是“左边先滑完、右边再补跳一下”。
- 左 pane、右 pane 和中缝必须同时参与位移；用户应感知为同一块工作台结构在重新分配宽度。
- 收起后只剩锚点按钮，按钮位置在展开态与收起态之间保持稳定，不做明显跳位。
- 工具条到内容区的过渡优先使用同色系边缘、浅内阴影和柔和明暗变化，不接受突兀黑线。
  - 悬浮 `返回顶部` 按钮只在右区滚动后出现，固定在右下角覆盖层里。

## 滚动模型

  - 桌面端由页面壳层锁定整页滚动：`html/body` 与 `site-main` 不承担主内容滚动。
- 主滚动发生在工作台内部：
  - 有左栏页面：左侧筛选与右侧内容各自独立滚动；
  - 无左栏页面：只有右侧主内容滚动。
- 筛选页右区继续承担：
  - 筛选后回到摘要区顶部；
  - 随机排序不强制回顶；
  - 从详情返回时恢复 query 和右区滚动位置。
- 非筛选页统一使用右区悬浮回顶入口，不操作 `window.scrollY`。

## 页面接入边界

- 页面只负责把内容映射到 `toolbarLead / toolbarPrimary / toolbarActions / sidebarHeader / sidebar / contentHeader / children`。
- 壳层本身统一负责：桌面锁滚、抽屉持久化、左右内滚容器、悬浮回顶覆盖层和 sidebarless 行为。
- 没有左侧内容的页面使用“隐藏左栏”模式，不另建页面外壳。

## 页面归位映射

工作台壳层覆盖全站桌面端页面；有左栏页面左右内滚，无左栏页面只保留右区主滚动。

| 页面 | 左顶部区 | 左主体区 | 右顶部区 | 右主体区 |
| --- | --- | --- | --- | --- |
| Champions | 条件 badge、清空、提示 | champions 筛选组件 | 标题、metrics、筛选摘要、显示全部/随机/复制链接 | 视觉档案、结果卡、空态 |
| User Heroes | 条件 badge、清空、提示 | 用户英雄筛选组件 | 标题、metrics、筛选摘要、显示全部/随机/复制链接 | 用户英雄结果卡、空态 |
| Illustrations | 条件 badge、清空、提示 | 立绘筛选组件 | 标题、metrics、筛选摘要、显示全部/随机/复制链接 | 立绘结果网格、空态 |
| Pets | 条件 badge、清空、提示 | 魔宠筛选组件 | 标题、metrics、筛选摘要、显示全部/随机/复制链接 | 魔宠结果网格、空态 |
| Variants | 条件 badge、清空、提示 | 变体筛选组件 | 标题、metrics、筛选摘要、显示全部/随机/复制链接 | campaign / adventure 分组结果、空态 |
| Formation | 布局状态、提示 | 布局搜索、场景类型、当前布局摘要、布局库 | 标题、当前布局、已放置英雄数、复制链接 | 草稿提示、画板编辑、阵型摘要、保存方案 |
| Planner | 隐藏 | 隐藏 | 标题、metrics、复制链接 | 计划配置、评估结果 |
| Presets | 隐藏 | 隐藏 | 标题、总数、可恢复数、复制链接 | 范围说明、方案列表、空态 / 编辑 / 删除 / 恢复 |
| Search | 隐藏 | 隐藏 | 标题、metrics、复制链接 | 搜索结果列表、空态 |
| User Data | 隐藏 | 隐藏 | 标题、当前导入方式、解析状态、复制链接 | 导入边界、导入工作台 |
| Champion Detail | 隐藏 | 隐藏 | 返回入口、标题、当前章节、复制链接 | 卷宗头部、章节内容、详情侧栏 |
| Animation Audit | 隐藏 | 隐藏 | 标题、metrics、复制链接 | 动画审计结果、空态 |

- `Illustrations Audit`（`/illustrations/audit`）是 `/illustrations` 前缀下的子路由条目，`Champion Detail`（`/champions/:championId`）是 `/champions` 下的详情子路由条目，`Planner Evaluate`（`/planner/evaluate`）是 `/planner` 下的评估子路由条目，均为独立页级 Route。
- `isWorkbenchRoute`（`src/app/workbenchRoutes.ts`）统一覆盖全部工作台页面与子路由（`/champions/:championId`、`/illustrations/audit`、`/planner/evaluate`）；桌面锁滚、头部紧凑与主内容高度锁定挂在这条路由判定上。

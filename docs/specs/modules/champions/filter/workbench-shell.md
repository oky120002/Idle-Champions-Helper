# 英雄筛选工作台壳层

- 作用：记录 `Champions` 工作台的局部状态约束；全站通用结构与页面归位映射见 `docs/specs/modules/shared-components/page-workbench-design.md`。

## 目标与非目标

### 目标

- `Champions` 页采用“上方全站导航 + 下方统一工作台大壳”的双层结构。
- 筛选规则、结果卡、详情跳转和 URL 同步契约独立于壳层；壳层只负责布局、滚动模型和顶部工具栏组织。
- 桌面端工作台风格：左抽屉有明显前后层次，右主区是更实的内容画布，左右都能独立滚动。
- 以仓库内规范和参考图锁定可复核的结构与状态。

### 非目标

- 不重写 `Champions` 的筛选业务规则、结果卡契约与详情跳转协议。
- 不引入新的筛选业务维度，也不改 `HashRouter` / GitHub Pages 兼容方式。
- 不复制桌面系统红绿灯或整套原生窗口细节；只借“壳层关系”和“工具栏合并”的结构语言。

## 参考态

### 展开态

![Champions 工作台展开态参考](./assets/workbench-expanded-reference.png)

- 左侧抽屉完整存在，带中等玻璃感和独立前景层次。
- 顶部工具栏虽然分成左段和右段，但视觉上必须连成一条。
- 右主区要比左抽屉更深、更实，形成“资料画布”而不是普通网页卡片。

### 收起态

![Champions 工作台收起态参考](./assets/workbench-collapsed-reference.png)

- 左抽屉主体、边框和残余 gutter 全部退场。
- 只保留一个紧凑展开入口，不保留整条窄边轨。
- 展开态位于右工具栏左端的一组信息左移贴边，右端动作继续保持右对齐。

## 工具栏合并规则

- 工具栏分成三块语义区：`展开入口 / 左侧识别信息`、`页面主标题与上下文`、`稳定动作区`。
- `展开态`
  - 左块宽度跟随抽屉宽度。
  - 右块与左块无缝拼接，不能看起来像两条分开的栏。
  - 左块承接页面标记与筛选状态，右块左端承接页面标题和上下文说明，右块右端只保留稳定动作。
- `收起态`
  - 左块压缩成紧凑入口按钮。
  - 原右块左端信息补位到入口按钮后方，整体仍保持左对齐。
  - 原右块右端动作不应因为收起而整体漂移。

## 面板归位映射

- 左顶部区：筛选状态 badge、`清空全部`、简短提示。
- 左主体区：现有 `ChampionsPrimaryFilters` + `ChampionsAdditionalFilters`，不改规则，只重排表面和节奏。
- 右顶部区：页面标题、稳定工具栏动作（`显示全部/收起`、`随机排序`、`复制当前链接`）；不重复展示筛选命中 badge。
- 右结果头：compact metrics 与当前筛选摘要，不承担高频操作。
- 右主体区：结果 metrics + 结果卡网格（卡片内嵌立绘背景）+ 空态。

## 滚动模型

- 桌面端：工作台使用固定可视高度；左抽屉和右主区各自滚动。
- 右主区滚动容器负责承载顶部说明、结果操作、视觉档案和结果卡列表。
- 筛选变更、默认窗口展开/收起后，右主区回到顶部摘要区。
- 随机排序不强制回顶。
- 进入详情再返回时，恢复的是右主区滚动位置，不恢复整页 `window.scrollY`。
- 移动端退化为单列普通网页滚动，不维持桌面工作台高度锁定。

## 实施边界

- 工作台壳层的全站统一实现在 `src/components/workbench/` 下的 `PageWorkbenchShell`；共性细节见 `docs/specs/modules/shared-components/page-workbench-design.md`。
- 筛选字段组件、结果卡组件和视觉档案组件在 `src/pages/champions/` 与 `src/features/champion-filters/`，跨页复用。
- 参考图是结构示意，用来锁定状态关系，不作为像素级验收基线。

## 验收口径

- 桌面端存在统一外层工作台大壳。
- 展开态与收起态都具备清晰的工具栏合并效果。
- 收起后左抽屉主体完全消失，右主区无残余空白轨。
- 左右面板可独立滚动，快捷按钮只作用于右主区。
- 文档、图片和实际实现的状态命名一致。

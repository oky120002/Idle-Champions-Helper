...
- 定位：Idle Champions 最佳阵型自动推算工具；在”当前条件 + 目标 + 限制”下，自动推算最优阵型配置，过程可解释、结果可验证。
- 成败不取决于页面数量，而取决于数据底座、规则表达、个人数据映射和推算精度。
...
| 暂不优先 | 社区社交 / 排行榜 / 内容运营需求 |
| 必须做 | 结构化数据底座、可过滤英雄 / 冒险 / 限制查询、阵型编辑、自动推算最优阵型、阵容保存与比较 |
| 不做 | 全玩法逐区完整模拟、用户系统 / 社区系统、照搬 Byteglow / Kleho 的大百科矩阵 |

## Planner

- 自动推算模块是产品的核心能力，详见 `docs/specs/modules/planner/README.md`。
- planner 自动推算当前条件下的最优阵型，保持过程可解释、结果可验证，不做无法追溯的黑盒。

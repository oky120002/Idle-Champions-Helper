...
| 必须做 | 结构化数据底座、可过滤英雄 / 冒险 / 限制查询、阵型编辑、限制下的候选英雄筛选、阵容保存与比较 |
| 不做 | 黑盒最优解站、全玩法逐区完整模拟、用户系统 / 社区系统、照搬 Byteglow / Kleho 的大百科矩阵 |

## Planner

- 自动计划模块是产品的核心能力之一，详见 `docs/specs/modules/planner/README.md`。

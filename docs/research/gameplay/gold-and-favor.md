...
**可信度**：⚠️ 待确认 — 金币掉落公式和神恩计算公式来自社区逆向推导，`game-rules.json` 部分验证（`health_gold_ratio: 0.65`）但指数 0.304 等无直接字段
...
最终金币 = 基础金币 × 金币寻获量（含神恩加成、英雄加成、祝福加成、药水等全部乘性叠加）。
...
活动（Highharvestide、Liars' Night、Simril 等）和时间门给予临时神恩，活动结束后按以下比率转换为永久神恩：

- 活动转换：`加成 = 10% × log10(活动神恩)`
- 时间门转换：`加成 = 2.5% × log10(Mystra 神恩)`（活动速率的 1/4）
...
`patron-perks.json` 中 4 位赞助者共有 7 个金币相关 perk：
...
| 赞助者金币 perk | `patron-perks.json → perks[] → effects[].effectString: "gold_multiplier_mult"` |
...
- **神恩指数**：Wiki 的 `(gold/5e4)^0.304` 等公式为社区逆向推导，游戏数据中未直接暴露神恩计算公式
- **神恩转换率**：`10% × log10(event_favor)` 为社区逆向，游戏数据中未直接暴露

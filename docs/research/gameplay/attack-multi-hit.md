...
**可信度**：✅ 已确认 — 游戏数据直证（`numTargets` + `add_attack_targets` 字段），无社区依赖
...
护甲敌人每格有伤害阈值，每次命中最多碎一格（溢出伤害不结转）。系数 0.33 的英雄每发伤害更低，可能达不到碎甲阈值 → **系数对碎甲有影响**，不能忽略。详见 [armored-enemies.md](./armored-enemies.md)。

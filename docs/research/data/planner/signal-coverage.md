...
- per_hero_expr：`291` 条，已解析 `219`（`75.3%`），未解析 `72`（几乎全为数值表达式，见 `docs/specs/modules/planner/expression-evaluator.md`，或 `HasEffect`/`GetUpgradeUnlocked` 等运行时状态）。
...
- 数值表达式（`min` / `max` / `floor` / `GetUpgradeAmount` / `levels_past_softcap` / `AverageILevels` / `NumEquipmentWithMinimumRarity` 等）——当前没有统一数值求值器，`parseHeroPredicate` 有意返回 `null`。
...
parser 对布尔谓词的覆盖已接近饱和；剩余未解析根因是缺少统一数值求值器，或依赖当前静态上下文不具备的运行时状态。`EligibleForPatron(...)`、`!HasEffect(...)`、裸 `base_attack_cooldown`、私有 stack 和动态公式均保持 warning，不做静态猜测。

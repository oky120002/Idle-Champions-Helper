...
| 来源 | 已建模（主） | 未建模 / 部分建模 |
|---|---|---|
| 英雄技能（hero-abilities，164 英雄 / 9818 信号） | carry 5949 + support 2277 全接（globalDps×2026、heroDps carry×5934 + support×197、health、gold、crit、vulnerability、damageReduction）+ 20 baseCrit（A2）；信号数 2026-08-01 实测（旧 10993/6261/4731 为 e053b759 移除 baked 装备源前快照，未同步） | unsupported 1592 无 parser（health_add×411、set_ultimate_attack×163、buff_ultimate×8 英雄技能源…，见 §5） |
...
| 专精（specialization-catalog，115 spec） | 11 kind 全量注入、全建模（heroDps×83、vulnerability×11、crit×12、health×4、gold×6、damageReduction×8；2026-08-01 修复 atd_b1e5f3a2c7 后实测——原 heroDps×142/vuln×21 含 loot/feat 源 overcount wrapper 已移出 catalog，仅留 ability 源 16 wrapper） | cooldownReduction×12、attackSpeedMult×5 已解析未消费（speed 维度） |
...
**E. 深度审计补录（2026-08-01 英雄维度全扫，5 个机制家族 inventory 原漏记；全依赖动态触发/跨冒险存档，静态评分不可消费）**：
...
**量级判断**：speed/cooldown 维度已解析未消费是**最大真实缺口**；装备 `buff_upgrade` owned wrapper 通道已接（B1-e，曾误判"未建模"，实为双重计数，见 `modeling-pitfalls.md`）；modron/favor/药水需先解决私有存档（userdetails）导入通道；§5E 五家族依赖动态触发/跨冒险存档，静态评分不可消费，登记不建模。
...
| 里程碑 | 范围 | 价值 | 成本 / 前置 |
|---|---|---|---|
| **M0 ✅ 已落地** | DPS 五通道 hero_dps/global_dps/health/gold/crit + `buff_upgrade` owned-aware wrapper（B1-a~e） | 主评分载体 | 已完成 |

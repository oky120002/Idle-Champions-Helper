...
| 装备（loot-catalog，4044 item） | hero_dps×160 + global_dps×688 + health×104 + gold×12（equipmentMult，enchant `(1+enchant/250)` 缩放；global_dps/gold 为 global-scope placement-aware per-hero，health 为 hero-scope per-carry） | **大头未接**：buff_upgrade×1824、reduce_ultimate_cooldown×608、buff_ultimate×272、crit×8 |
...
- 装备：`buff_upgrade`×1824、`reduce_ultimate_cooldown`×608、`buff_ultimate`×272、`crit`×8——**装备加成大头未接**（equipmentMult 接 hero_dps+global_dps+health+gold，余维度未接）。

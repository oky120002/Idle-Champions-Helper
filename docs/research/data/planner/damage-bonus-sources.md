...
- 效果：已建模来源变精确；整体偏差由「负负得正的接近」转为「纯低估」（未建模缺口显现）。
- 符合核心原则：已建模的全对，未建模的明确标注。
- 连带：golden（ADR 0015）会红——需重定位为「结构回归守护」（非精度标尺）或更新基线；同步 simulator.md 公式 + steadyStateScoring.test.ts + breakdown.factors 契约。
- D2 协同：删死产物 global-buffs.json（TODO `atd_7b9e1f4c2a`），避免误导。
...
1. **选 A / B / C？**（推荐 A：让已建模来源精确，停止负负得正；B 的未建模补建可作后续专项）
2. **golden 测试（ADR 0015）重定位**：精度标尺 → 结构回归守护，还是更新基线？
3. **未建模源补建优先级**：vulnerability 条件生效（生产有数据）是否最先？

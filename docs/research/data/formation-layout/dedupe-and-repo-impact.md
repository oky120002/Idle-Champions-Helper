# 官方布局：去重结果与边界事实

- 作用：沉淀唯一布局数量、布局 ID 生成方式与当前链路边界。
- 阵型布局来源决策（官方 definitions 自动提取为主、人工覆写为辅、手工 MVP 不作主链路）已落地，见 `specs/modules/formation/scope-and-inputs.md`（`formations.json` = 自动提取的唯一布局库）；本文件只留去重与边界事实。

## 去重结果

截至 2026-04-13 的快照：

- 带阵型的 `campaign`：28
- 带阵型的普通 `adventure`：283
- 带阵型的 `variant`：554
- 总上下文：865
- 去重后唯一布局：157
- 槽位数量分布：9 槽 1 个、10 槽 152 个、11 槽 3 个、13 槽 1 个

当前 `formations.json`（`updatedAt: 2026-07-28`）包含 `160` 个去重布局：9 槽 1 个、10 槽 155 个、11 槽 3 个、13 槽 1 个。当前发布产物不保留去重前的 campaign / adventure / variant 分项计数，因此不把旧快照的总上下文数量当作当前事实。

仓库实现使用“归一化槽位串 + `sha1` 截断”生成稳定布局 ID；签名至少应包含 `column / row / x / y / adjacentSlotIds`。

## 当前边界

- `language_id=7` 对部分新冒险或时空门条目仍可能回退英文。
- 官方中文个别文本可能有翻译质量问题；当前策略是优先保留官方返回，再为必要缺口补人工覆写。
- 当前接入的是“布局数据自动提取”，不是“按战役 / 冒险 / 变体筛选布局”的完整交互方案（交互方案见 `specs/modules/formation/scope-and-inputs.md`）。

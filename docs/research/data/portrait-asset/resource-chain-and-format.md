...
- 当前 `champions.json`（`updatedAt: 2026-08-10`）的 `165 / 165` 名可上阵英雄都带本地 portrait 路径。
...
- 2026-04-13 raw 快照对当时 161 名英雄做过完整格式核查：头像资源成功数 `161`、缺失映射数 `0`、包装头长度全部 `24` 字节、真正图片尺寸全部 `256x256`。当前产物证明 165 名英雄均有本地头像，但不把旧快照的全量尺寸统计外推到新增英雄。

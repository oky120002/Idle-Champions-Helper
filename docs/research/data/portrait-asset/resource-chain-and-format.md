...
- 2026-04-13 当天核实到的 161 名可上阵英雄中，`161 / 161` 都能映射到官方头像资源，没有缺失项（当前数据已增长至 163，结论不变，需重跑覆盖统计）。
...
- 截至 2026-04-13 的快照：161 名可上阵英雄做完整核查后，头像资源成功数 `161`、缺失映射数 `0`、包装头长度全部 `24` 字节、真正图片尺寸全部 `256x256`（当前数据已增长至 163，结论不变，需重跑覆盖统计）。

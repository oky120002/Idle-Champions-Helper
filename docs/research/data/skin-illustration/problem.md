...
- 根因链路：从 `public/data/v1/champion-visuals.json` 选 `large / base / xl` 候选 -> 下载 `mobile_assets` -> 用 `scripts/data/mobile-asset-codec.mjs` 解包 -> 直接写入 `public/data/v1/champion-illustrations/heroes|skins/*.png`（把 atlas 当成立绘写盘，缺少离线组装）。

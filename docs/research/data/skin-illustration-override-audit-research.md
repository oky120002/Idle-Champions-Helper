...
- 日期：2026-04-16
- 状态：基于当前 `public/data/v1/champion-illustrations.json`、已生成立绘产物和当日新增的抽样脚本，当前仍有效。
...
- 在已落地 4 条人工 override 后，非默认 `sequence / frame` 的皮肤仍只有这 4 个：
  - `368`：半精灵变异哨兵 -> `sequence 1 / frame 11`
  - `133`：星运伊芙琳 -> `sequence 0 / frame 7`
  - `473`：飞升阿斯代伦 -> `sequence 1 / frame 30`
  - `520`：舞厅蔚 -> `sequence 1 / frame 7`
- 因此这轮仍只能使用“同英雄内部尺寸偏差 + 主题分类 + 已复核样例回避”去筛人工优先级，而不能直接靠字段过滤判断错图。
- 当前这一轮高优先级样本已经基本跑完；把已复核样本并入默认 `reviewedSkinIds` 后，再按 `risk >= 4` 复跑，已没有剩余未复核的高优先级候选。
...
所以现在仍没有任何一个简单字段可以直接指向“这张图错了”；只能继续做候选排序 + 人工复核。
...
- 作用：按英雄分组，取“英雄本体 + 全部皮肤”的尺寸中位数做基线，再对每个皮肤计算宽度偏差、高度偏差、面积偏差和 `visiblePieceCount`；同时做主题分类。
- 主题分类当前覆盖：
  - 明显缩小主题：`毛绒 / 宝宝 / 玩偶 / chibi / action figure`
  - 明显扩展主题：`魔冢 / 飞升 / 夺心魔 / 神裔 / 龙枪 / 星运 / 女巫之光 / 宇宙梦魇 / 巨人`
...
4 条规则都已写入 `scripts/data/champion-illustration-overrides.json`。
...
| `390` | 夺心魔维康妮亚 | 夺心魔主题天然外扩，候选只轻微收窄，不建议写 override |
...
- 结构偏差高但仍不应改的代表样本：`332`（魔冢 BBEG），它说明“偏差大”不自动等于“图错了”。

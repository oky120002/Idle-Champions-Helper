...
- `CONTEXT.md`：项目统一语言术语表，只记领域术语定义与别名（游戏术语 ↔ 代码标识符映射）。
...
| 根级独立文档（`CONTEXT.md` 等非 README 根级权威来源） | <= 240 行 | 241-320 行 | 321-420 行 | > 420 行 |
| 叶子文档 | <= 120 行 | 121-180 行 | 181-260 行 | > 260 行 |

行数只是警戒线；只要一个文档同时承载多种读者意图，就算没超行数也应继续拆。根级独立文档是权威单一来源（如 `CONTEXT.md` 统一语言术语表），查询型用途决定一跳命中最优、拆分代价高于普通叶子文档，故默认保留阈值更宽；根规则文件（`AGENTS.md` / `CLAUDE.md`）只留指针，不受此表约束。
...
- 根规则文件（`AGENTS.md` / `CLAUDE.md`）不得复制细则已展开的读取顺序、结构命名、拆分规则、体量预算、样式规则；此类内容只进对应 `docs/specs/guidelines/*` 细则，根规则文件至多留一行指针。

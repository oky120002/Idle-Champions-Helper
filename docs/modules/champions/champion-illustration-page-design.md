...
- 页面真正需要稳定展示的单元是：`161` 个英雄本体立绘 + `672` 个皮肤立绘 = `833` 个展示单元。
- 不需要把 `portrait / base / large / xl` 等全部技术槽位原样公开；按技术槽位全量算会到 `3010` 个资源单元，超出页面真实需求。
- 当前仓库首版落地产物已简化为 `public/data/v1/champion-illustrations/heroes/*.png` 与 `public/data/v1/champion-illustrations/skins/*.png`；文中 `thumbs / display / webp` 是后续可演进的正式页面资源设计，不代表 2026-04-16 的唯一现状实现。
...
- GitHub Pages 还有限额：发布站点 `<= 1 GB`、源仓库建议 `<= 1 GB`、每月 `100 GB` 软带宽；因此最优策略是只发布页面要消费的衍生图，而不是把所有原始技术槽位一股脑塞进仓库。
...
- 保留 `public/data/<version>/champion-visuals.json` 作为官方资源定位基座，继续保存 `graphicId / sourceGraphic / sourceVersion / remoteUrl / delivery` 等元数据。
- 新增 `public/data/<version>/champion-illustrations.json` 作为页面消费清单；它只回答“页面最终怎么展示”，不复写底层官方资源定位逻辑。
...
- 脚本职责：读取 definitions 与 `champion-visuals.json`；枚举全部展示单元；下载候选资源；按 `delivery` 解包；在 `SkelAnim` 场景下解析 piece / 帧数据；渲染候选静态 pose；计算真实宽高与内容边界；选择页面主来源；裁透明边；输出页面衍生图与展示清单；写出体积和尺寸审计摘要。
...
- 第一条原则：只发布 `833` 个页面展示单元，不发布全部 `3010` 个技术槽位原图。
- 第二条原则：页面统一消费衍生图，不直接引用原始 PNG。
- 第三条原则：脚本必须输出总体积报告，并在接近上限时失败；建议把站点构建产物接近 `850 MB` 设为保守失败阈值，为其他资源预留余量。
- 新数据版本上线时，应允许清理旧版本立绘二进制，避免仓库历史和工作树持续膨胀。

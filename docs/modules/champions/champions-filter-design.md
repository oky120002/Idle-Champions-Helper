...
3. 筛选区
   ...
   - 低频标签条件按两组折叠收纳：
     - 身份画像：种族 / 性别 / 阵营
     - 来源与特殊机制：职业 / 获取方式 / 特殊机制
   - 已选条件条保留在侧栏内，支持单独清空某个维度或一键清空全部
4. 结果区
   - 空态提示
   - 当前已选条件摘要
   - 当前展示数提示
   - 可按需展开的“英雄视觉档案”面板：展示本体立绘、官方头像、皮肤立绘槽位与远端元数据
   - 英雄结果卡片列表
   - 长列表提供结果区快捷滚动入口，但按钮应按当前位置动态显隐，避免常驻打扰
   - 桌面到超宽屏按视口宽度分档放宽主内容；在约 `2545px` 视口下，主内容区应接近 `2200px`，并尽量让结果区稳定落在约 `6` 列，再随屏宽逐级递减到移动端 `1` 列

这样可以保持信息密度足够高，同时把高频和低频条件拉开，避免桌面端因为筛选项过多而把结果区压得过窄、过跳。

补充说明：

- “英雄视觉档案”不是另起一张详情页，而是内嵌在筛选结果区上方的工作台。
- 工作台只展示站内可用的本地头像参考与基座元数据，不在浏览器端请求任何官方资源。
- 若后续需要接入实际图片，必须先走构建期同步，把资源落到站内本地静态目录后再展示；当前页面只保留 `graphicId / sourceGraphic / sourceVersion / delivery / uses` 等基座字段。
...
- 特殊机制
  - 当前基于 `tags` 归类得到
  - 当前按 `站位相关 / 控制效果 / 专精方向` 三组展示，帮助用户更快理解
  - 支持多选
...
- roles
- affiliations
- 结构化属性概览（当前基于 `tags` 拆分为种族 / 性别 / 阵营 / 职业 / 获取方式 / 特殊机制 / 其他）

...
| `PageWorkbenchShell` | `src/components/workbench/PageWorkbenchShell.tsx` | 全站统一桌面工作台壳层 | 统一合并工具条、左抽屉开合、无左栏隐藏模式、双栏内滚和桌面锁滚 | `ChampionsPage.tsx`、`UserHeroesPage.tsx`、`IllustrationsPage.tsx`、`PetsPage.tsx`、`VariantsPage.tsx`、`FormationPage.tsx`、`PresetsPage.tsx`、`PlannerPage.tsx`、`UserDataPage.tsx`、`ChampionDetailPage.tsx`、`AnimationAuditPage.tsx` |

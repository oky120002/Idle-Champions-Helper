...
> **boss vulnerability 匹配**（第八轮发现 → 第十二轮根因修复）：3 个 `monsterTags:['boss']` 的 vulnerability 因 `scenario.enemyTypes` 不含 boss 静默失效。根因不是缺 `is_boss` 字段——raw `monster_defines` 用 `tags:["boss"]` 标记 boss（808/2326 怪），与 fiend 同维；真因是 `GENERIC_MONSTER_TAGS` 把 boss 当"非类型 tag"排除出 enemyTypes。已从 `GENERIC_MONSTER_TAGS` 移除 boss（`normalize-adventures.ts`），enemyTypes 词表与 vulnerability 词表对齐（见 `data-normalization-guidelines.md` §3）。**产物生效需重跑 normalize 同步 `variants.json`→`scenarios.json`**——当前 variants.json 停留 M2 9.2 快照，未含本轮修复。

...
> **boss vulnerability 匹配**（第八轮发现 → 第十二轮根因修复）：3 个 `monsterTags:['boss']` 的 vulnerability 因 `scenario.enemyTypes` 不含 boss 静默失效。根因不是缺 `is_boss` 字段——raw `monster_defines` 用 `tags:["boss"]` 标记 boss（808/2326 怪），与 fiend 同维；真因是 `GENERIC_MONSTER_TAGS` 把 boss 当"非类型 tag"排除出 enemyTypes。已从 `GENERIC_MONSTER_TAGS` 移除 boss（`normalize-adventures.ts`），enemyTypes 词表与 vulnerability 词表对齐（见 `data-normalization-guidelines.md` §3）。**产物已生效**：commit 26bf32c4 重生官方基座至 2026-07-24 快照，`variants.json`/`scenarios.json` 1413 条 enemyTypes 全部含 boss，3 个 boss vulnerability 信号下游可命中。**UI 层消费 enemyTypes 时须过滤 boss**（`NON_DISPLAY_ENEMY_TAGS`，见 `variant-labels.ts`）——boss 非种族，且已在 specialEnemyCount 独立展示，避免「敌人类型」chip/过滤器/占比污染（第十三轮审计修复）。
...
> 全量校验（1405 variant）：61 slot-occupying 模板匹配 + 12 手工补 override（具名列表 / "of the" 间隔 / Monodrone+Duodrone / 龙占格 / 宾客粉丝猫 NPC / 向导）= 68 scenario 产 `occupiedSlotCount`；其余 flavor 文本进 warning。`RESTRICTION_OVERRIDES` 提供手工补机制（`restrictions-parser.ts`）。wired 进 `buildOfficialScenarioModel` → `scenario.occupiedSlotCount` + 具体 warning 替代原「自由文本未解析」。
...
> **第十轮审计修正（静默丢数据 + ZH 变量排除）**：① EN/ZH 双侧模板漏匹配 7 类非英雄占格 variant（措辞超出模板：动词变位 takes/taking up 不含 take up 子串、number 与 slots 间插 formation、"take up space" 无 slots、具名实体无数词）→ 补 override（v1124 龙 3 / v1261 龙 escort 3 / v1629 龙 2 / v414 宾客 2 / v444 粉丝 2 / v1589 猫 3 / v682 NPC 2 / v96 向导 1）；override 子串须全量扫 1405 variant 核爆半径——具名专属串（"rudolph van richten and his ally"）排在泛化串（"barovian wedding"）前，否则 v1977/78/79（Rudolph 是 forcedHeroes 英雄，应 0）被误命中为 2。② ZH 变量递增占格（"每经过 N 区域额外一格"，v70/v116/v127/v296/v461）原被 "一格" 直配 regex 误判为固定 1 → 补 `ZH_AREA_INCREMENT_RE` 排除；须同时排除位置轮换（"改变位置/变换位置/阵型中移动"等明确短语 = 固定 N 格换位置，计数不变），否则 v241（固定 2 格换位置）被 "每经过" 误清零。**第十一轮修正**：原标记含孤立「移动」误匹配 v296「无法被移动」（forcedHeroes 英雄锁定），改为只收明确短语（见 `data-normalization-guidelines.md` §10）。

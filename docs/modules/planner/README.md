...
- **[`evolution-plan.md`](./evolution-plan.md)**：阵型模拟器演进规划总纲（三层架构 + 16 阶段进度追踪 + TDD 硬约束 + 里程碑分组 + 命名修正 A1 + 加成聚合调研）。新 session 读总纲了解全貌，再按里程碑读对应 `milestone-{2..4}-*.md` 执行步骤（M1 已完成）。命名以通用符号去 Planner 为准。

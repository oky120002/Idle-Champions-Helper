...
- `getRawFilters` 此前在 `signalSemantics.ts`（生产，读 4 源）与 `signal-coverage.ts`（报告，只读 2 源）各有一份且已漂移，覆盖率少统计 `target_filters_or`/`targets` 来源。已统一为 `signalSemantics.ts` 单一来源，报告复用。

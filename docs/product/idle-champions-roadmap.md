...
- 难点不是单个英雄强度，而是 seat 冲突、阵型站位、Patron / Trials / Time Gate / Variants 限制，以及装备、传奇、feats、pigments、favor、blessings、modron 等成长系统叠加。
- 玩家真实问题是“这局在我当前条件下怎么组更省时间”；工具要支持组合、过滤、验证、保存，最好还能读个人进度。
- 单纯百科价值有限；能减少切游戏、切 wiki、切表格、切第三方站的往返，才有明显收益。
...
- 推荐定位：个人成长导向的阵型决策台。
- 一句话：在“当前拥有条件 + 当前目标 + 当前限制”下，快速给出可执行阵容骨架、候选池和成长优先级。
- 真正决定成败的不是页面数量，而是数据底座、规则表达、个人数据映射和可解释推荐。
...
- 一句话 MVP：按限制筛英雄、按布局排阵、保存方案、逐步接入个人账号数据的工具站。
- 首页：进入查询、阵型、目标模式。
...
- 成功指标：自己愿意把它当主查询入口；能明显减少多站切换；1 分钟内完成一次筛选与保存；“这个限制能上谁”的查询时间明显下降。

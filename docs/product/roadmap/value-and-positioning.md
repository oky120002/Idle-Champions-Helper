...
| 第一阶段不做 | 黑盒最优解站、全玩法逐区完整模拟、用户系统 / 社区系统、照搬 Byteglow / Kleho 的大百科矩阵 |

## Planner 更新

- 2026-05-03 更新：自动计划模块进入分阶段设计，详见 `docs/modules/planner/README.md`。
- 新边界是“不做黑盒最优解”，而不是永远不做推荐或模拟；第一纵切只承诺本地优先、可解释、可验证的稳态推墙伤害推荐。

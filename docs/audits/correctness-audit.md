...
| 项 | 动作 | ROI | 影响面 | 进度 |
|----|------|-----|--------|------|
| 外部加成池分裂 | 外部 global_dps/hero_dps 注入对应池 addPercent（加法），同步 spec/test/breakdown | 中高（修 IC 语义偏差 + 排名扭曲，但被欠估掩盖） | scoreFormation 局部 + simulator.md + steadyStateScoring.test + PlannerBreakdown | **未启动**；须与未建模源补全协同 |
| spec/TODO 漂移修正 | `simulator.md` 公式 + `equipmentMult` 注释 + TODO `atd_3cb8df390e`（仍提已删的 `theoreticalLootMult/ownedEquipMult` 只收 global_dps，实际 3849d295 已改收 hero_dps） | 低（文档准确性） | docs + TODO | **未启动** |

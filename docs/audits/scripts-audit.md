...
| 3 | **build 派生产物无 schema 校验**：`validate-data-schemas` 只校验 5 个产物（champion-details + champions/adventures/variants/patrons），但管线产出 ~16 个。planner 核心消费的 `hero-abilities.json`/`scenarios.json`/`specialization-catalog.json`/`feat-catalog.json`/`global-buffs.json`/`effect-definitions.json`/`loot-catalog.json` 等 **均无 zod schema 与 CI gate**。champion-details（build 输入）已校验 + build 函数 TS 类型管结构，残差风险在 build 逻辑用 `as`/optional-but-expected 字段导致的形状偏差 | 仿 `collection-schemas.ts` passthrough 哲学，为 hero-abilities / scenarios / 两个 catalog / global-buffs 补 zod schema（钉消费方核心字段），接入 `validate-data-schemas` 的 collectionChecks；schema ↔ build 类型配 passthrough 双源守护（沿用 `collectionSchemaSync.test.ts` 模式） | 中（catches build 逻辑形状 bug + 作契约文档；但 TS 类型已挡大部分结构性问题，ROI 低于 #1）。改动面中等（每产物一个 schema + 守护） | `src/domain/types/build-product-schemas.ts`（6 产物 schema）+ `validate-data-schemas.ts` 接入 + `build-product-schema-sync.test.ts` 类型守护 | **✅ 收口（2026-08-01）**：6 个 planner 核心产物（hero-abilities/scenarios/loot-catalog/effect-definitions/feat-catalog/specialization-catalog）补 passthrough schema（钉消费方核心字段，含 A2 加的 baseCritChancePercent），接入 `collectionChecks`（174 目标 0 失败）+ 类型层 sync 守护（钉死字段 ⊆ 消费 interface）。**剔除 global-buffs.json**：审计发现其为死产物（`build-models.ts:128` 写出但 src/scripts/tests 全量扫描无运行时 fetch，被 patron-perks/blessings 重算路径取代），无消费方则 schema 无保护对象；✅ 已删（A1 Phase A Commit 2，2026-08-01，TODO `atd_7b9e1f4c2a` 收口：删 global-buffs.json + build-models 写出 + 死管线 patron-perk-signals.ts）。同步修正 recommendationEngine.ts / steadyStateScoring.ts 2 条引用死加载路径的失效注释。其余 ~9 个视觉/动画/manifest 产物非 planner 正确性依赖，TS 类型已管结构，不在范围 |
...
| **buff_upgrade 排除**（§11，13944ac8） | `collectEffectEntries`（effect-helpers.ts:737）`isPlainBuffUpgrade && isAbilitySource && !isDynamicStacks → continue`：ability 源静态 buff_upgrade（已烘进满级 effect_string snapshot）不派生计入目标值信号，避免 22× 双重计数。保留三类运行时修饰：stacks_multiply 动态 / 复杂 wrapper / 外部源 loot-feat-legendary。修复在位，注释证据链完整（蔚善良榜样 4^7 实测） |
...
| **schema 同步**（4 collection + 6 build 派生产物） | `collection-schemas.ts`（champions/adventures/variants/patrons）+ `build-product-schemas.ts`（hero-abilities/scenarios/loot-catalog/effect-definitions/feat-catalog/specialization-catalog）passthrough 钉核心字段 + `collectionSchemaSync.test.ts`/`build-product-schema-sync.test.ts` 类型层守护（schema 钉死字段 ⊆ 消费 interface）+ `validate-data-schemas` CI exit 1（174 目标）。单源防漂移全覆盖 |
...
- **P0 清零**：本轮无 P0。
- **P1 登记**：3 项（signal-coverage 假门 ✅ 已收口 2026-08-01 / build 派生产物无 schema ✅ 已收口 2026-08-01 / 192 MB .bin 无 LFS），均带动作 + ROI + 影响面 + 决策点。剩 192 MB .bin 一项需部署架构决策。

...
| bannedHeroes 非空场景 | **0/1413**（§5 死代码） |
...
| 8 | forced∩banned 同英雄冲突 | forced 让英雄进候选，`checkFormationLegality` 判 `bannedChampion`→全部 beam 结果非法→`no-legal-recommendation`（banned 胜出，不放非法阵型） | ✗（本轮补测） | 当前 bannedHeroes 恒空→不可达（§5） | 健康（防御） |
...
| `bannedHeroes` 死代码 | 确认游戏无场景级英雄黑名单后**全链路删除**（scenario 字段 + `bannedChampion` 合法性分支 + `banList` 约束类型 + forced∩banned 测试）；或若确有黑名单机制则补 build parser | 中（清理死代码减误读；CLAUDE.md 全链路删除纪律） | plannerModel/formationLegality/recommendationEngine/variantConstraints + buildScenarioModels + 本轮测试 | 游戏是否有 blacklist 场景机制（whitelist 已覆盖限制需求） |
...
- 空 placements / 候选不足 / 未知 heroId / forced∩banned 均有可见 blocker 或 warning，无错误推荐。
...
- bannedHeroes 分布：`jq -r '[.items[].bannedHeroes|length]|group_by(.)|map({len:.[0],n:length})|.' public/data/$v/scenarios.json`

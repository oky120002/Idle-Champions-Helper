...
3. 每条违规先读 message，按 docs/plans/2026-08-system-lint-fix-rules.md 对症。
...
- 形式主义规则遗留清理（149 文件，净减 ~570 行）：
  - **已完成**：
    - void block body `() => { fn() }` → 箭头简写 `() => fn()`（no-confusing-void-expression 遗留，~400 处）
    - `[...].some(Boolean)` → 回 `||` 链（expression-complexity 遗留，1 处：illustration-model.ts）
    - 参数类型 `unknown` → 恢复原联合类型（max-union-size 遗留，1 处：championRoster.ts toNumber）
    - `return undefined` → bare `return`（no-inconsistent-returns 遗留；例外：函数末尾且 `noImplicitReturns` 要求的保留 `return undefined`，否则触发 `sonarjs/no-redundant-jump`）
    - 死 `eslint-disable` 注释：阶段 13 已清零（`--report-unused-disable-directives` 确认无残留）
  - **跳过**：
    - 私有字段 `readonly`：plannerCompute.ts 2 处未被 lint 改动（自然代码，非 prefer-readonly 遗留）

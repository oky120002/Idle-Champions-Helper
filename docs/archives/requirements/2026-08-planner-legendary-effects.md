...
升级等级放大效果值。需求资料记录的社区缩放公式（[Steam 英雄能力公式帖](https://steamcommunity.com/app/627690/discussions/0/4336482609599058214/)）：

```
effect = gear_base × (1 + (ilvl - 1) × 0.004)
```

该公式和实际数据口径仍需在阶段二/三前核定；当前评分阶段一使用 `base × level` 的保守线性缩放。升级消耗：每级 +153 鳞片（线性递增）+ 神恩（消耗当前 90%，最低需求 e10→e100 指数递增）。
...
- **神恩（Divine Favor）管理**：神恩类型与战役绑定，属账号管理范畴非阵型评估

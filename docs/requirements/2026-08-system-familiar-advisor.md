...
基于玩家当前阵型和已拥有的魔宠数量，推荐最优魔宠分配方案——哪些放场上点击、哪些放英雄身上自动升级、哪些放杀招栏、哪些管药水。以独立面板展示在阵型编辑器旁边。
...
| 杀招 | 通常不需要 | 4 只放关键杀招（爆发窗口） |
| 英雄升级 | 速度英雄优先 | 全阵型英雄 |

### 数据现状

- `pets.json`：345 只魔宠目录（获取方式、宝石价格、赞助者商店）✅——但**纯视觉元数据**（图标、获取方式），**无效果/能力数值**（点击速率、阶梯阈值等运行时常量不在游戏公开数据中，仅社区记载）
...
- **阵型上下文**：当前阵型中的英雄列表（哪些需要自动升级？有没有速度英雄？有没有需要杀招爆发的 DPS 英雄？）
- **魔宠总数**：玩家拥有的魔宠数量

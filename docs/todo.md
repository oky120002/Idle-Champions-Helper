...
- planner distance wrapper: `buff_upgrade_mult_by_distance_from_source` 与 `_mult` 变体已命中归一化（各 1 signal，复用 `per_slot_distance_from_source` 合同）；`buff_upgrade_per_column_behind_source_mult` 仍无样本且 `isBuffUpgradeKind` 未登记，官方数据出现时补登记并复用现有合同。
...
- planner 长期扩展（超出 evolution-plan v4 的 16 阶段，待产品规划立项）：balanced scoring（混合伤害/存活/速度/可获得性）、step simulation（逐区/击杀/时间窗口/动态堆叠）、多队伍/Trials/Time Gate、event/season/temporary buff 投影、manual parameter panel（手动覆盖金币预算/装备/feat/传奇/专精）。speed/survival/modron/patron 已在 evolution-plan 阶段 5/7/11/14 覆盖，不在此列。

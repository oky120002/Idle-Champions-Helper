...
- planner 推荐引擎 Planner* 别名 shim：`src/domain/planner/plannerModel.ts` 把 abilities 层的 `HeroAbilitySignal`/`HeroAbilityProfile` 等 re-export 为 `PlannerEffectSignal`/`OfficialPlannerHeroModel` 等旧名供引擎消费，与 A1"通用符号去 Planner、引擎内部引用通用数据用新名"决策有出入。当前为保引擎词汇稳定的过渡 shim；全链路改名（placementFit/steadyStateScoring/recommendationEngine/recommendationTypes + 测试）零行为收益、面大，待命名统一专项处理。

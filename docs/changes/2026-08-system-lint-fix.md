...
- [ ] 阶段 12: src/pages 剩余全部（实测 ~542 违规 / 113 文件）—— 补清阶段 8/9/11 漏的子目录（champion-detail 82/animation-audit 54/variants 78/champions 92/pets 62/illustrations 44）+ 原范围（user-data 25/user-heroes 17/presets 16/home 4/根页面 58）。分 4-5 轮派 Agent（每轮 3 个，每 Agent ≤~50 违规），优先大目录；每轮独立 commit。根因：阶段 8/9/11 各自 eslint 范围只含"该批文件"未覆盖目录全部（阶段 11 另有 fd 误判子目录不存在）
